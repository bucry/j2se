...
public class TTCommon {
	...
	public static TTCardType jugdeType(List<TTCard> list) {
		...
		if (len >= 5) {// 现在按相同数字最大出现次数
			...
			// 求出各种数字出现频率
			TTCommon.getMax(card_index, list); // a[0,1,2,3]分别表示重复1,2,3,4次的牌
			// 3带2 -----必含重复3次的牌
	...
	public static void get123(List<TTCard> list, TTModel model) {
		List<TTCard> del = new Vector<TTCard>();// 要删除的Cards
		if (list.size() > 0
	...
	public static int checkCards(List<TTCard> c, List<TTCard>[] current,TTMain m) {
		...
		if (cType == TTCardType.c31 || cType == TTCardType.c32
			...
			List<TTCard> a1 = TTCommon.getOrder2(c); // 我出的牌
			List<TTCard> a2 = TTCommon.getOrder2(currentlist);// 当前最大牌
