...
public class TTTime extends Thread {
	TTMain main;
	boolean isRun = true;
	int i = 10;

	public TTTime(TTMain m, int i) {
		this.main = m;
		this.i = i;
	}
	...
	public void run() {
		...
		for (TTCard card2 : main.playerList[1])
		...
		if (main.time[1].getText().equals("抢地主")) {
			...
			openlord(true);
			second(2);// 等待五秒
			TTCommon.order(main.playerList[1]);
			TTCommon.rePosition(main, main.playerList[1], 1);
			main.publishCard[1].setEnabled(false);
			setlord(1);
		} else {
			// 电脑选地主
			if (TTCommon.getScore(main.playerList[0]) < TTCommon
				...
				TTCommon.order(main.playerList[2]);
				TTCommon.rePosition(main, main.playerList[2], 2);
				openlord(false);
				...
				TTCommon.order(main.playerList[0]);
				TTCommon.rePosition(main, main.playerList[0], 0);
				openlord(false);
	...
	public void ShowCard(int role) {
		...
		TTModel model = TTCommon.getModel(main.playerList[role], orders);
		// 待走的牌
		...
		if (main.time[(role + 1) % 3].getText().equals("不要")
			...
			if(role!=main.dizhuFlag)
			{
				...
				{
					if((TTCommon.jugdeType(main.currentList[(role+2)%3])!=TTCardType.c1||
							TTCommon.jugdeType(main.currentList[(role+2)%3])!=TTCardType.c2)
						&&main.currentList[main.dizhuFlag].size()<1)
						f=1;
					if(main.currentList[(role+2)%3].size()>0&&TTCommon.getValue(main.currentList[(role+2)%3].get(0))>13)
						f=1;
				}
				if(f==1){
			...
			TTCardType cType = TTCommon.jugdeType(player);
			// 如果是单牌
			if (cType == TTCardType.c1) {
				if (can == 1)
					model = TTCommon.getModel(main.playerList[role], new int[] {
							1, 4, 3, 2, 5 });
				AI_1(model.a1, player, list, role);
			}// 如果是对子
			else if (cType == TTCardType.c2) {
				if (can == 1)
					model = TTCommon.getModel(main.playerList[role], new int[] {
							2, 4, 3, 5, 1 });
				AI_1(model.a2, player, list, role);
			}// 3带
			else if (cType == TTCardType.c3) {
				AI_1(model.a3, player, list, role);
			}// 炸弹
			else if (cType == TTCardType.c4) {
				AI_1(model.a4, player, list, role);
			}// 如果是3带1
			else if (cType == TTCardType.c31) {
				if (can == 1)
					model = TTCommon.getModel(main.playerList[role], new int[] {
							3, 1, 4, 2, 5 });
				AI_2(model.a3, model.a1, player, list, role);
			}// 如果是3带2
			else if (cType == TTCardType.c32) {
				if (can == 1)
					model = TTCommon.getModel(main.playerList[role], new int[] {
							3, 2, 4, 5, 1 });
				AI_2(model.a3, model.a2, player, list, role);
			}// 如果是4带11
			else if (cType == TTCardType.c411) {
				AI_5(model.a4, model.a1, player, list, role);
			}
			// 如果是4带22
			else if (cType == TTCardType.c422) {
				AI_5(model.a4, model.a2, player, list, role);
			}
			// 顺子
			else if (cType == TTCardType.c123) {
				if (can == 1)
					model = TTCommon.getModel(main.playerList[role], new int[] {
							5, 3, 2, 4, 1 });
			...
			// 双顺
			else if (cType == TTCardType.c1122) {
				if (can == 1)
					model = TTCommon.getModel(main.playerList[role], new int[] {
							2, 4, 3, 5, 1 });
			...
			// 飞机带单
			else if (cType == TTCardType.c11122234) {
				AI_4(model.a111222, model.a1, player, list, role);
			}
			// 飞机带对
			else if (cType == TTCardType.c1112223344) {
		...
		if (list.size() > 0) {
			...
			for (int i = 0, len = list.size(); i < len; i++) {
				List<TTCard> cards = getCardByName(main.playerList[role],
						list.get(i));
				for (TTCard card : cards) {
					temp.add(card);
				}
			}
			temp=TTCommon.getOrder2(temp);
			for (TTCard card : temp) {
			...
			TTCommon.rePosition(main, main.playerList[role], role);
		} else {
			main.time[role].setVisible(true);
			main.time[role].setText("不要");
		}
		for (TTCard card : main.currentList[role])
			card.turnFront();
	}

	// 按name获得Card，方便从Model取出
	public List<TTCard> getCardByName(List<TTCard> list, String n) {
		String[] name = n.split(",");
		List<TTCard> cardsList = new Vector<TTCard>();
		int j = 0;
		for (int i = 0, len = list.size(); i < len; i++) {
	...
	// 顺子
	public void AI_3(List<String> model, List<TTCard> player, List<String> list,
			int role) {

		for (int i = 0, len = model.size(); i < len; i++) {
			String[] s = model.get(i).split(",");
			if (s.length == player.size()
					&& getValueInt(model.get(i)) > TTCommon.getValue(player
							.get(0))) {
	...
	public void AI_4(List<String> model1, List<String> model2,
			List<TTCard> player, List<String> list, int role) {
		// 排序按重复数
		player = TTCommon.getOrder2(player);
		...
		for (int i = 0; i < len1; i++) {
			...
			if ((s.length / 3 <= len2)
					...
					&& getValueInt(model1.get(i)) > TTCommon.getValue(player
							.get(0))) {
	...
	public void AI_5(List<String> model1, List<String> model2,
		...
		if (len1 < 1 || len2 < 2)
			return;
		for (int i = 0; i < len1; i++) {
			if (getValueInt(model1.get(i)) > TTCommon.getValue(player.get(0))) {
				list.add(model1.get(i));
				for (int j = 1; j <= 2; j++)
					list.add(model2.get(len2 - j));
			}
	...
	// 单牌，对子，3个，4个,通用
	public void AI_1(List<String> model, List<TTCard> player, List<String> list,
			int role) {

		for (int len = model.size(), i = len - 1; i >= 0; i--) {
			if (getValueInt(model.get(i)) > TTCommon.getValue(player.get(0))) {
				list.add(model.get(i));
				break;
			}
		}
	...
	public void timeWait(int n, int player) {
		...
		if (main.currentList[player].size() > 0)
			TTCommon.hideCards(main.currentList[player]);
